...
main(){
    ...
    while(1){ //simple check
        ...
        if (k>0 && k%2 == 0 && x < 0){
            ...
            else{ //Explanation: the program cannot handle negative powers of a root properly. Therefore, using the properties, it is converted to a favourable form.
                k = fabs(k);
                x = 1/x;
                break;
            }
        ...
        else{ //If all conditions are met, the check cycle stops and the program moves on
            break;
        }
...
RootCalc(double x, int k, double e, double y, double b){
    ...
    while(fabs(b) >= e){
        ...
        if (x == 0 || x==1){ //Basic case 1.
            printf("y = %f", x);
            break;
        }
        else if(k == 1){ //Basic case 2.
            printf("y = %f", x);
            break;
        }

        else{ //Çàãàëüíà ôóíêö³ÿ
            double yk = pow(y, k-1);
